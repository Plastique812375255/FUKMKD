...
# -*- coding: utf-8 -*-
"""
修正后的res文件提取工具 - 排除不存在的beep_1.au和beep_2.au文件
"""
import struct
import os
import sys
...
def extract_files_from_res(res_file_path, output_dir):
    """
    从res文件中提取所有的音频文件，排除beep_1.au和beep_2.au这两个不存在的文件
...
if __name__ == "__main__":
    # 检查命令行参数
    if len(sys.argv) < 2:
        print("用法: python3 extract_wav_files_corrected.py <res文件路径> [输出目录]")
        print("示例: python3 extract_wav_files_corrected.py sys/sound_1_vbc2_en/res extracted_wav")
        sys.exit(1)
    
    # 获取参数
    res_file_path = sys.argv[1]
    output_dir = "extracted_wav"
    if len(sys.argv) > 2:
